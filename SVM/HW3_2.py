...
def BalanceData(data):
    ...
    data_train = pd.concat(
        [data_norm_train, data_spam_train], ignore_index=True)
...
def NormalizeFeatures(features):
    """
    I'm providing this mostly as a way to demonstrate array operations using Numpy.
    Incidentally it also solves a small step in the homework.
    """
...
def get_roc_curve(y_test, y_prob, num_thresholds):
    thresholds = np.asarray(
        range(0, num_thresholds, 1)) / float(num_thresholds)
...
def accuracy_features_select(X_train, y_train, X_test, y_test, weights):
    ...
    for i in range(2, len(order_list), 1):
        ...
        accuracies_train.append(model.score(select_X_train, y_train))
        m = list(range(2, len(order_list), 1))
    return accuracies, m, accuracies_train
...
# fpr, tpr, thresholds = get_roc_curve(y_test, probas_[:, 1], 200)
# plt.plot(fpr, tpr)
...
probas_ = model.fit(X_train, y_train).predict_proba(X_test)
# # I don't really know why the coeficients
weights = abs(model.coef_)
# # weights = np.array([probas_[1]]).reshape(1, 57)  ## give the same features
# #order_list = weights.argsort()
# accuracies_train, m, accuracies_train = accuracy_features_select(X_train, y_train, X_test, y_test, weights)
# #plt.plot(m, accuracies_select) 
# plt.plot(m, accuracies_train)
# plt.title('Experiment 2: Accuracy vs m of Features')
# plt.ylabel('Accuracy')
...
# %%
# Experiment 3
accuracies_random, m, accuracies_train = accuracy_features_random(
    X_train, y_train, X_test, y_test, weights)
plt.plot(m, accuracies_random)
plt.plot(m, accuracies_train)
plt.title('Experiment 3: Accuracy vs m of Features')
plt.ylabel('Accuracy')
plt.xlabel('m Features')
plt.legend(('test', 'train'), loc=4)
plt.show
